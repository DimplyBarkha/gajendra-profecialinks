...
module.exports = {
  ...
  implementation: async (
    ...
    await context.evaluate(async() => {
      ...
      const basicNextBtn = document.querySelector('.ui-k6');
      try{
        if(pagination.hasChildNodes() && !basicNextBtn){
          const currPage = Number(document.querySelector('.b9g0.b9g2').textContent);
          const nextLink = document.createElement('a');
          nextLink.className = 'ui-k6 custom nav';
          nextLink.setAttribute('href', mainUrl.replace(/page=.*&/, `page=${currPage + 1}`));
