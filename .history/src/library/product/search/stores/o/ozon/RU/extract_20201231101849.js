    ...
    await context.evaluate(async() => {
      ...
      if(cardsItems){
        cardsItems.forEach((item, index) => {
          ...
          const itemRating = item.querySelector('.a1y6');
          ...
          if(itemRating) {
            let itemRatingValue = itemRating.getAttribute('title');
            itemRatingValue = Number(itemRatingValue).toFixed(1);
            itemRatingValue = itemRatingValue.toString();
            itemRatingValue = itemRatingValue.replace(/./, ',')
            itemDescr.setAttribute('oz-rating', itemRatingValue);
          }
        });
      }
