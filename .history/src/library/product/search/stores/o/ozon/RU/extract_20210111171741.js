...
module.exports = {
  ...
  implementation: async (
    ...
    await context.evaluate(async() => {
      ...
      let pageNum = 1;
      try {
        if(mainUrl.includes('category') && pageNum === 1){
          localStorage.setItem('LSisCatalog', 'ture');
          isCatalog = true;
        }
        console.log(isCatalog, pageNum, "- INFO");
        if(mainUrl.match(/page=\d*&/gmi)){
          const pageCount = mainUrl.match(/page=\d*&/gmi)[0].replace(/\D/gmi, '');
          console.log(pageCount);
          pageNum = +pageCount;
        }
        if (pageNum > 1){
          const prevPageIsCat = localStorage.getItem('LSisCatalog');
          console.log(prevPageIsCat, "PREV IS CAT");
          if(prevPageIsCat === 'ture' && !isCatalog){
            console.log('IS NOT CAT!')''
          }
        }
