    ...
    data.forEach(dataObj => {
        dataObj.group.forEach((fieldName, index) => {
            ...
            if(fieldName.aggregateRating) {
                let replace = ['.', '.'];
                let aggregateRatingValue = fieldName.aggregateRating[0].text;
                let fixedNumber = Number(aggregateRatingValue).toFixed(3);
                fixedNumber = fixedNumber.toString();
                fixedNumber = fixedNumber.replace(replace[0], replace[1]);
                fieldName.aggregateRating[0].text = `${fixedNumber}`;
                // fieldName.aggregateRating[0].value = +fixedNumber;
                // fieldName.aggregateRating[0].raw = `${fixedNumber}`;
            }
