...
async function implementation(inputs, parameters, context, dependencies) {
  ...
  if (stopConditionSelectorOrXpath) {
    const conditionIsTrue = await context.waitForFunction(
      (sel) => {
        try {
  ...
  if (nextLinkXpath) {
    ...
    const hasNextLink = await context.evaluate(
      ({ selector, uuid }) => {
        ...
        if (
          elem &&
          elem.singleNodeValue &&
          elem.singleNodeValue.nodeType === 1
        ) {
          // check the node type is element
          // @ts-ignore
  ...
  const success = openSearchDefinition
    ? false
    : await pager({
        ...inputs,
  ...
  console.log("Going to url new", url);
  try {
  ...
  if (resultsDivSelector) {
    // counting results
    const resultNB = await context.evaluate(
      (sel) => document.querySelectorAll(sel).length,
      resultsDivSelector
    );
    console.log(
      `The page has: ${resultNB} results. Pagination continues: ${!!resultNB}`
    );
    return !!resultNB;
  }
...
module.exports = {
  parameters: [
    ...
    },
    {
      ...
      description:
        "CSS selector for what to wait to change (if in-page pagination)",
    },
    {
      name: "spinnerSelector",
      description:
        "CSS selector for a spinner to wait to disappear (if in-page pagination)",
    },
    {
      name: "loadedSelector",
    ...
    },
    {
      ...
      description:
        "if this selectors returns an element then the pagination is brought to an end",
    },
    {
      name: "resultsDivSelector",
      description:
        "alternative to noResultsXPath, if the count is zero the pagination stops",
    },
    {
  ...
  get path() {
    ...
    return (
      this.tempPath ||
      `${actionjsPath}/stores/\${store[0:1]}/\${store}/\${country}/paginate`
    );
  },
  tempPath: "",
  // @ts-ignore
