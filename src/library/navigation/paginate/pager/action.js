...
async function implementation (
  ...
  if (mutationSelector) {
    ...
    await Promise.all([
      ...
      context.waitForMutuation(mutationSelector, { timeout: 500000 }),
    ]);
    return true;
  }
  ...
  if (nextLinkSelector) {
    ...
    await context.clickAndWaitForNavigation(nextLinkSelector, {}, { timeout: 500000 });
    if (loadedSelector) {
      await context.waitForSelector(loadedSelector, { timeout: 500000 });
    }
    if (loadedXpath) {
      await context.waitForXPath(loadedXpath, { timeout: 500000 });
    }
    return true;
  }
  return false;
