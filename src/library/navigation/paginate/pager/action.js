...
async function implementation (
  ...
  if (spinnerSelector) {
    ...
    }, { timeout: 1000000 }, spinnerSelector);
  ...
  if (mutationSelector) {
    ...
    await Promise.all([
      ...
      context.waitForMutuation(mutationSelector, { timeout: 1000000 }),
    ]);
    return true;
  }
  ...
  if (nextLinkSelector) {
    ...
    await context.clickAndWaitForNavigation(nextLinkSelector, {}, { timeout: 1000000 });
    if (loadedSelector) {
      await context.waitForSelector(loadedSelector, { timeout: 1000000 });
    }
    if (loadedXpath) {
      await context.waitForXPath(loadedXpath, { timeout: 1000000 });
    }
    return true;
  }
  return false;
