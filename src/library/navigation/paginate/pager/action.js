...
async function implementation (
  ...
  if (spinnerSelector) {
    ...
    }, { timeout: 300000 }, spinnerSelector);
  ...
  if (mutationSelector) {
    ...
    await Promise.all([
      ...
      context.waitForMutuation(mutationSelector, { timeout: 300000 }),
    ]);
    return true;
  }
  ...
  if (nextLinkSelector) {
    ...
    await context.clickAndWaitForNavigation(nextLinkSelector, {}, { timeout: 300000 });
    if (loadedSelector) {
      await context.waitForSelector(loadedSelector, { timeout: 300000 });
    }
    if (loadedXpath) {
