...
async function implementation (
  ...
  if (spinnerSelector) {
    ...
    }, { timeout: 170000 }, spinnerSelector);
  ...
  if (mutationSelector) {
    ...
    await Promise.all([
      ...
      context.waitForMutuation(mutationSelector, { timeout: 170000 }),
    ]);
    return true;
  }
  ...
  if (nextLinkSelector) {
    ...
    await context.clickAndWaitForNavigation(nextLinkSelector, {}, { timeout: 170000 });
    if (loadedSelector) {
      await context.waitForSelector(loadedSelector, { timeout: 170000 });
    }
    if (loadedXpath) {
      await context.waitForXPath(loadedXpath, { timeout: 170000 });
    }
    return true;
  }
  return false;
