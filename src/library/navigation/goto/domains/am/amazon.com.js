  ...
  implementation: async ({ url }, parameters, context, dependencies) => {
    ...
    const run = async () => {
      ...
      if (lastResponseData.code !== 200) {
        ...
        return context.reportBlocked(lastResponseData.code, 'Blocked: ' + lastResponseData.code);
      }
      ...
      const wrongLocale = await context.evaluate(async function () {
        ...
        console.log('localeEl', detailsLocaleEl.snapshotLength);
        if (!!detailsLocaleEl.snapshotLength || !!searchLocaleEl.snapshotLength) {
          console.log('wrongLocale', 'true');
          return 'true';
        } else {
          console.log('wrongLocale', 'false');
          return 'false';
        };
      });
