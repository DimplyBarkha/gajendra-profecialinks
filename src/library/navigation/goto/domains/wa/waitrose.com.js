...
module.exports = {
  ...
  implementation: async ({ url, zipcode, storeId }, parameters, context, dependencies) => {
    ...
    const user_agent = JSON.parse(allUserAgentString[Math.floor(allUserAgentString.length * Math.random())].config).userAgent + ' ' + Math.random().toString(36).substring(2, 15);
    ...
    async function isCaptcha () { return await context.evaluate(async () => !!document.querySelector('#recaptcha')); };
    let RETRIES = 3;
    while ((await isCaptcha()) && RETRIES--) {
    ...
    if (await isCaptcha()) {
      throw new Error('Could not solve captch');
    }
    console.log(zipcode);
    if (zipcode) {
