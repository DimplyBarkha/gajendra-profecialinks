module.exports = {
  ...
  implementation: async (inputs, parameters, context, dependencies) => {
    ...
    await context.setInputValue(
      'input[class="search-input Locator-input js-locator-input"]',
      `${inputs.zipcode}`
    );
    ...
    const retailerId = await context.evaluate(async (storeid) => {
      const query = `#js-yl-${storeid}`;
      await new Promise((resolve, reject) => setTimeout(resolve, 50000));
      const regex = /\d+/
      if (query) {
        return document.querySelector(query)
          ? document.querySelector(query).id.match(regex)
          : "";
      }
    }, storeid);

    const retailerZipcode = await context.evaluate(async (storeid) => {
      //return !document.querySelector(`input[value="${storeid}"]`);
      const query = `#js-yl-${storeid} span.c-address-postal-code`;
      await new Promise((resolve, reject) => setTimeout(resolve, 50000));
    ...
    await context.waitForSelector(
      'div.search-page-content__products-tab-content ul li a[data-auto-id="linkProductTitle"]'
    );
    const newUrl = await context.evaluate(function () {
      const tempUrl = document.querySelector(
        'div.search-page-content__products-tab-content ul li a[data-auto-id="linkProductTitle"]'
      );
      if (tempUrl) {
    ...
    await context.evaluate(addHtml, retailerId, "retailerid");
    await context.evaluate(addHtml, retailerZipcode, "retailerzipcode");
