  ...
  implementation: async ({ url, zipcode }, parameterValues, context, dependencies) => {
    ...
    const solveCaptchaIfNecessary = async function () {
      ...
      while (await isCaptcha() === 'true' && captchas < MAX_CAPTCHAS) {
        captchas++;
        await context.waitForSelector('iframe[role="presentation"]', { timeout: 60000 });
        if (backconnect) {
    ...
    await context.goto(url, { timeout: 120000, waitUntil: 'load', checkBlocked: false, block_ads: false, js_enabled: true, css_enabled: false, random_move_mouse: true });
  },
};
