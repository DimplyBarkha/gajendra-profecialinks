...
async function implementation (inputs, parameters, context, dependencies) {
  ...
  function loopCategory (obj, parent) {
    ...
    for (let i = 0; i < Object.keys(obj).length; i++) {
      ...
      if (parent) {
        categories.push({ name: `${parent.name}|${obj[key].text}`, url: obj[key].url });
      } else categories.push({ name: obj[key].text, url: obj[key].url });
  ...
  if (categoriesArr.length) {
    await context.evaluate(({ categoriesArr }) => {
      ...
      for (let i = 0; i < categoriesArr.length; i++) {
        ...
        addHiddenDiv('categoryUrl', `https://www.matas.dk${categoriesArr[i].url}`, newDiv);
      }
    }, { categoriesArr });
  } else {
