    ...
    for (const row of group) {
      ...
      if (row.thumbnail) {
        row.thumbnail.forEach((thumbnailItem) => {
          thumbnailItem.text = thumbnailItem.text.replace(/\/\//gm, '');
        });
      }
      if (row.id) {
        row.id.forEach((idItem) => {
          idItem.text = idItem.text.replace(/.*p\/(.*)/gm, '$1');
        });
      }
      if (row.productUrl) {
        row.productUrl.forEach((productUrlItem) => {
          productUrlItem.text = 'https://www.schubiger-online.ch' + productUrlItem.text;
        });
      }
      rankCounter += 1;
      if (!row.sponsored) {
