async function implementation (inputs, parameters, context, dependencies) {
  ...
  await context.evaluate(() => {
    const priceSelector = document.querySelectorAll('.mq-product-prices');
    let price;
    const productUrlSelector = document.querySelectorAll('.mq-product-img>div>a');
    let productUrl;
    ...
    for (let i = 0; i < allProducts; i++) {
      ...
      if (productUrlSelector[i].href !== undefined && productUrlSelector[i].href !== null) {
        productUrl = 'https://merqueo.com' + productUrlSelector.href;
      }
