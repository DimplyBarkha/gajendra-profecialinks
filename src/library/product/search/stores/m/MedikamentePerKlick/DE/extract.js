async function implementation (
  ...
  await context.evaluate(async function () {
    ...
    document.querySelectorAll('div.boxProduct.rowEven,div.boxProduct.rowOdd,div.boxProduct specialProduct').forEach(node => {
      addHiddenDiv(node, `ii_rank_${pageStartIndex}`, pageStartIndex);
      const priceSelector = node.querySelector('dd.yourPrice,dd.yourPrice price');
      // @ts-ignore
