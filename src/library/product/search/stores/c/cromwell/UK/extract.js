...
async function implementation(
  ...
  var results = await context.evaluate(async function () {
    ...
    function addHiddenDiv(id, content, index) {
      ...
      const originalDiv = document.querySelectorAll('div div.FamilyDetailsPanel.FamilyDetailsPanel_ListView')[index];
      originalDiv.appendChild(newDiv);
      console.log("child appended " + index);
    }
    const result = [];
    const product = document.querySelectorAll('div div.FamilyDetailsPanel.FamilyDetailsPanel_ListView');
    // select query selector and loop and add div
    for (let i = 0; i < product.length; i++) {
      addHiddenDiv('page_url', URL, i);
      const productUrl = document.querySelectorAll('div.FamilyDetailsPanel_Image.searchImage a');
      if (product[i].querySelector("span[data-e2e='productSku']") === null) {
        result.push({
        ...
        result.push({
          url: 'https:' + productUrl[i].getAttribute('href'),
          code: product[i].querySelector("span[data-e2e='productSku']")
        })
      }
    }
    return result;
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (results[i].code === '') {
      ...
      const productCode = await context.evaluate(async function () {
        const productCode = document.querySelector("span[data-e2e='productSku");
        return productCode;
      });
  ...
  await context.evaluate(async function (results) {
    try {
      var index = 0;
      (document.querySelectorAll('div.FamilyDetailsPanel_Image.searchImage a')).forEach((node) => {
