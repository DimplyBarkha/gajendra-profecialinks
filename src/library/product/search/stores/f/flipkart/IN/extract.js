...
async function implementation(
  ...
  await context.evaluate(async function () {
    ...
    await fetch(window.location.href, { method: 'GET' })
      .then(r => r.text())
      .then(html => {
        const result = GetTagByIdUsingRegex('script', 'is_script', html);
        const outerHTML = result && result[0] ? result[0] : '';

        document.body.insertAdjacentHTML('beforeend', outerHTML);
      });

    const str = document.body.querySelector('#is_script') ? document.body.querySelector('#is_script').innerHTML : '';
    const obj = JSON.parse(str.split('window.__INITIAL_STATE__ = ').slice(-1)[0].trim().slice(0, -1));
    const products = obj.pageDataV4.page.data[10003];
