...
async function implementation (
  ...
  if (!url && openSearchDefinition) {
    ...
    // @ts-ignore
    const searchUrl = await context.evaluate(() => document.querySelector('#searchUrl') && document.querySelector('#searchUrl').innerText && document.querySelector('#searchUrl').innerText.match(/(.*).html$/));
    if (!searchUrl) {
      url = template
...
module.exports = {
  ...
  parameterValues: {
    ...
    openSearchDefinition: {
      indexOffset: 0,
      template: 'https://www.labaie.com/search?q={searchTerms}&search-button=&lang=fr_CA&start={offset}&sz=24',
    },
    domain: 'labaie.com',
    zipcode: '',
  },
