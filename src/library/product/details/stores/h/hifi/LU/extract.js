...
module.exports = {
  ...
  implementation: async (inputs,
    ...
    const seeMoreProductInfoCSS = 'a.brand-content-link'; // anchor tag
    const iframeDivContainerCss = '#loadbeeTabContentId'; // div has API key to make call to fetch iframe url
    const iframeIdCSS = '#loadbeeIframeId'; // iframe id css
    const noInformationAvailableXpath = `//div[contains(@class, 'content-provider-content')]//*[contains(text(), 'aucune information supplémentaire est disponible')]`;

    const waitForIframeDiv = async (loadbeeIframeIdCSS) => {
      try {
        await context.waitForSelector(loadbeeIframeIdCSS);
        console.log(`loadbeeIframeId found: ${loadbeeIframeIdCSS}`);
      } catch (err) {
        console.log(err);
      }
    };

    const checkNoInfo = async (xpath) => await context.evaluate((xpath) => !document.evaluate(xpath, document, null, XPathResult.BOOLEAN_TYPE, null).booleanValue, xpath);
    // no info => false else true
    try {
      await context.waitForSelector(seeMoreProductInfoCSS, { timeout: 10000 });
      console.log('seeMoreProduct button found, clicking it...');
      await context.click(seeMoreProductInfoCSS); // loads iframe upon click

      const isNoInfoavailable = await checkNoInfo(noInformationAvailableXpath);
      if (!isNoInfoavailable) {
        await waitForIframeDiv(iframeIdCSS); // wait for iframe load
        await context.evaluate(async (iframeDivContainerCss, seeMoreProductInfoCSS) => {
          const iframeDiv = document.querySelector(iframeDivContainerCss);
          if (iframeDiv) return; // check if iframe div loaded, return

          const seeMoreAtag = document.querySelector(seeMoreProductInfoCSS);
          if (seeMoreAtag) { // else click see more info node again
            console.log(`clicking again seeMoreAtag : ${seeMoreAtag}`);
            seeMoreAtag.click();
          }
        }, iframeDivContainerCss, seeMoreProductInfoCSS);
      } else {
    ...
    if (!await checkNoInfo(noInformationAvailableXpath)) {
      ...
      console.log(`No additional info were found, will not navigate...`);
    }
