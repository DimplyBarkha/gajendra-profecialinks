...
module.exports = {
  ...
  implementation: async (inputs,
    ...
    if (!src) {
      ...
      try {
        await context.waitForSelector(iframeDivContainerCss, { timeout: 10000 });
        ...
        console.log(`Div with API key not found, API will not hit.. ${iframeDivContainerCss}`);
    ...
    if (src) {
      try {
        ...
        hasComparisionTable = await context.evaluate(async function () {
          let  hasComparisionTable = '';
            const checkHasComparisonTable = document.querySelector('div.table-wrapper table') ? "true1"  : "false1" ;
