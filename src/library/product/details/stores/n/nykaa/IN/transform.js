  ...
  for (const { group } of data) {
    for (const row of group) {
      ...
      if (row.secondparagraph) {
        const text = row.secondparagraph.map((element) => element.text.trim()).join(' ');
        row.secondparagraph = [{ text }];
      }
      if (row.description) {
        const text = row.description.map((element) => element.text.trim()).join(' ');
      ...
      if (row.variantInformation) {
        let variantImageTitle = row.variantInformation[0].text;
