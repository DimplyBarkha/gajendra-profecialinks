...
module.exports = {
  ...
  implementation: async function (
    ...
    await context.evaluate((inputs) => {
      const is404 = document.querySelector('p.page_nt_found');
      if (!is404) {
        ...
        if (wholeVariants.length == 0) {
          ...
          if (inputs && inputs.url) {
            const urlArray = inputs.url.split('?');
            if (urlArray.length > 0) {
              const baseUrl = urlArray[0];
              console.log(baseUrl);
              let skuId = baseUrl.match(/p\/\d+/g)[0];
              if (skuId) {
                skuId = skuId.match(/(\d+)/g)[0];
                const appendUrl = `${baseUrl}?skuId=${skuId}`;
                const appendDiv = document.createElement('div');
                appendDiv.className = 'variantinfo';
                appendDiv.setAttribute('varianturl', appendUrl);
                appendDiv.setAttribute('variantid', skuId);
                document.querySelector('body').appendChild(appendDiv);
              }
            }
          }
