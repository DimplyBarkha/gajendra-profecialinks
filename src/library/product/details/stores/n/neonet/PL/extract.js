...
async function implementation(inputs, parameters, context, dependencies) {
	...
	async function checkVideo(productUrl) {
		return await context.evaluate(async function (productUrl) {
			...
			let videoThumbnail = document.querySelectorAll(
				'button[class*="commonCss-thumb-Dw9"] button[class*="playIcon"]'
			);
			...
			const videoSelector = document.evaluate(
				'//iframe[@title="movie"]',
				document,
				null,
			...
			if (videoSrc) {
				...
				if (videoSrc.includes('youtube')) {
					...
					console.log(
						'This is not a youtube video - not sure if it is playable or not'
					);
				}
	...
	let onlyText = await context.evaluate(async function () {
		let inBoxArray = [];
		const inBoxText = document.querySelectorAll(
			'div.productSpecificationCss-row-14w span'
		);
	...
	async function checkmanufacturerContent() {
		return await context.evaluate(async function () {
			const manufacturerIFrameSelector = document.evaluate(
				'//div[@id="rich-content-ext"]/iframe[@class="iFrame-iframe-2gX"]',
				document,
				null,
				XPathResult.FIRST_ORDERED_NODE_TYPE,
				null
			).singleNodeValue;
			const manufacturerIFrameSrc = manufacturerIFrameSelector
				? manufacturerIFrameSelector.src
				: '';
	...
	await context.evaluate(() => {
		if (document.querySelector('button[class*=productRichContent]')) {
			...
			document
				.querySelector('button[class*=productRichContent]')
				.scrollIntoView({ behavior: 'smooth' });
			document
				.querySelector('nav[class*="sectionsNavigation"]')
				.scrollIntoView({ behavior: 'smooth' });
			document
				.querySelector('button[class*=productRichContent]')
				.scrollIntoView({ behavior: 'smooth' });
	...
	try {
		...
		console.log(
			'Either manufacturer image not present or it is under iframe',
			e.message
		);
	}
	...
	async function fetchManufacturerContentIframe(url) {
		...
		return await context.evaluate(async function () {
			...
			const inBoxImageText = document.querySelectorAll(
				'div.description-row > div.description-col-md-3'
			);
			...
			const inBoxImagesList = document.querySelectorAll(
				'div.description-row > div.description-col-md-3 img'
			);
	...
	async function addContentToDOM(
		manContentObj,
		manufacturerContentLink,
		videoLink,
		onlyText
	) {
		await context.evaluate(
			async function ([
				...
				if (manufacturerContentLink) {
					...
					for (
						let i = 0;
						i < manContentObj.manufacturerImageArray.length;
						i++
					) {
						addHiddenDiv(
							'added-manufacturer-images-' + i,
							imageUrlSfx + manContentObj.manufacturerImageArray[i]
						);
					}
					addHiddenDiv(
						'added-manufacturer-description',
						manContentObj.manufacturerDescription
					);

					for (let i = 0; i < manContentObj.inBoxImageArray.length; i++) {
						addHiddenDiv(
							'added-inBox-images-' + i,
							imageUrlSfx + manContentObj.inBoxImageArray[i]
						);
					}
					...
					if (manContentObj.hasComparisionTable) {
						addHiddenDiv(
							'checkhasComparisionTable',
							manContentObj.hasComparisionTable
						);
					}
				}
				if (manufacturerContentLink) {
	...
	let manContentObj;
	if (manufacturerContentLink) {
		manContentObj = await fetchManufacturerContentIframe(
			manufacturerContentLink
		);
		try {
			await context.goto(inputs.url, {
