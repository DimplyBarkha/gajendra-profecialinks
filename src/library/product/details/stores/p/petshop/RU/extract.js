...
module.exports = {
  ...
  implementation: async (inputs, { transform }, context, { productDetails }) => {
    ...
    const productDescription = await context.evaluate(async () => {
      ...
      return document.evaluate('//button[div[text()="Описание"]]/following-sibling::div[position()=1]', document, null, XPathResult.STRING_TYPE, null).stringValue;
    });
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      await context.evaluate(
        async ({ variantsTotal, productDescription }) => {
          ...
          // const variantObj = productObj.offers.find((offer) => offer.artnumber === variantId);

          const imagesList = document.createElement('ol');
          imagesList.id = 'images';
          const allImages = productObj && productObj.images ? productObj.images.filter((image) => image.type === 'image') : [];
          ...
          const servingSize = caloriesText.match(/(\d+) ?(г)/) ? caloriesText.match(/(\d+) ?(г)/)[1] : '';
          const servingSizeUom = caloriesText.match(/(\d+) ?(г)/) ? caloriesText.match(/(\d+) ?(г)/)[2] : '';
          ...
          const carbText = document.evaluate('//tr[td[contains(. , "натрий")] and contains(. , "г") and not(contains(. , "%"))]', document, null, XPathResult.STRING_TYPE, null).stringValue;
          ...
          const proteinText = document.evaluate('//tr[td[contains(. , "белок") or contains(. , "белки")] and contains(. , "г") and not(contains(. , "%"))]', document, null, XPathResult.STRING_TYPE, null).stringValue;
          ...
          const calciumText = document.evaluate('//tr[td[contains(. , "витамин C")] and not(contains(. , "%"))]', document, null, XPathResult.STRING_TYPE, null).stringValue;
          ...
          const magnesiumText = document.evaluate('//tr[td[contains(. , "железо")] and not(contains(. , "%"))]', document, null, XPathResult.STRING_TYPE, null).stringValue;
