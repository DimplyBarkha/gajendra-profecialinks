...
module.exports = {
	...
	implementation: async (
		...
		await context.evaluate(async function () {
			...
			const netWeight = getAllXpath(
				"//div[@class='col-lg-8 col-md-12 col-sm-12 pl-20 pr-20']//p/text()",
				'nodeValue'
			);
			...
			const description = getAllXpath(
				"//div[@class='shortdec']/p[not(contains(@class,'pt-10 pointer'))]/text()",
				'nodeValue'
			).join('|');
			...
			const manufacturerDescription = getAllXpath(
				"//div[@id='descr']//div[@class='col-lg-8 col-md-12 col-sm-12 pl-20 pr-20']/div/p[not(img)]",
				'innerText'
			);
			console.log('manufacturerDescription fetched ' + manufacturerDescription);
			addElementToDocument(
				'added_manufacturerDescription',
				manufacturerDescription
			);

			const manufacturerImages = getAllXpath(
				"//div[@id='descr']//div//p//img/@src | //tr//td//img/@src",
				'nodeValue'
			).join('|');
			...
			const alternateImages = getAllXpath(
				"//div[@class='ms-thumb-list ms-dir-h']//div[@class='ms-thumb-frame' and position()>1]//img/@src",
				'nodeValue'
			);
			console.log(
				'this is the value for the alternate images------------------------------------------------------------',
				alternateImages
			);
			const finalArray = [];
			if (alternateImages.length > 0) {
				alternateImages.forEach((alternateImage, index) => {
					let completeImageUrl = `https:${alternateImage}`;
					finalArray.push(completeImageUrl);
				});
			}
			// if (
			// 	alternateImages != null &&
			// 	alternateImages.length > 0 &&
			// 	alternateImages.includes('|')
			// ) {
			...
			const inTheBoxUrl = getAllXpath(
				"//div[@id='descr']//div[@class='row']//strong[contains(.,'כלול באריזה')]/ancestor::p/following-sibling::table//img//@src | //p[contains(.,'מה בתיבה:') or (contains(.,'כלול באריזה'))]//img//@src | //div[@id='descr']//p[(br)]//img[contains(@style,'width:350px')]/@src  | //div[@id='descr']//p//img[contains(@alt,'סוללה נשלפת') or contains(@alt,'דייסון וי 11 החדש')]/@src | //div[@id='descr']//div[@class='row']//strong[contains(.,'אביזרים בערכה')]/ancestor::p/following-sibling::table//td//img/@src",
				'nodeValue'
			).join('|');
			const finalArray1 = [];
			if (
				inTheBoxUrl != null &&
				inTheBoxUrl.length > 0 &&
				inTheBoxUrl.includes('|')
			) {
			...
			const productOtherInformation = getAllXpath(
				"//div[@class='item panel panel-default panel-body']//span | //div[@class='item panel panel-default panel-body']//p",
				'innerText'
			).join('|');
			console.log('productOtherInformation fetched ' + productOtherInformation);
			addElementToDocument(
				'added_productOtherInformation',
				productOtherInformation
			);

			const specifications = getAllXpath(
				"//div[@id='descr']//div[@class='col-lg-8 col-md-12 col-sm-12 pl-20 pr-20']/div/p/strong[contains(text(),'מפרט')]/parent::p/text()",
				'nodeValue'
			).join('||');
