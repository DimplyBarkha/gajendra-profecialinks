...
async function implementation(inputs, parameters, context, dependencies) {
  ...
  await context.evaluate(async function () {
    ...
    var bUrl = "https://www.lancome.com.au/";
    var vurls = [];
    var varShade = getAllXpath(
      '//div[contains(@class,"product-variation-shade__item")]/a/@data-pid',
      "nodeValue"
    );
    var varSize = getAllXpath(
      '//div[contains(@class,"product-variation-size__item")]/@data-productid',
      "nodeValue"
    );
    var vcdid = getAllXpath(
      '//div[contains(@class,"product-variation-shade__item")]/a/@data-id',
      "nodeValue"
    );
    var vsdid = getXpath(
      '//input[@id="product_selectedsize"]/@value',
      "nodeValue"
    );
    ...
    if (varShade.length > 1) {
      for (var i = 0; i < varShade.length; i++) {
        vurls.push(bUrl+varShade[i] +'.html');
      }
      ...
      for (var i = 0; i < varSize.length; i++) {
        vurls.push(bUrl + varSize[i] + ".html");
      }
      for (var i = 0; i < vurls.length; i++) {
        addElementToDocument("vurls", vurls[i]);
      }
