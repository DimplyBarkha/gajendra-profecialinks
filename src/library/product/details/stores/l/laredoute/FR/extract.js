...
async function implementation(inputs, parameters, context, dependencies) {
	...
	await helper.ifThereClickOnIt('#footer_tc_privacy_button', {
		wait: 30000,
		click: 3000,
	});
	await helper.ifThereClickOnIt('div.productDescriptionShowMore-container', {
		wait: 30000,
		click: 3000,
	});
	...
	await helper.ifThereClickOnIt(
		'div.flixmedia_expandBtn.flixmedia_expandBtn--more',
		5000
	);
	...
	await context
		.waitForSelector(cssEnhancedContentContainer, { timeout: 10000 })
		.catch((error) =>
			console.log('Enhanced content not loaded : #pdpFlixmediaZone')
		);
	await context.evaluate(async (css) => {
		const node = document.querySelector(css);
		node
			? node.removeAttribute('style')
			: console.log('Selector not found: CSS => ', css);
	...
	const variantArray = await context.evaluate(async function () {
		...
		if (
			document.querySelectorAll('div.clearfix.custom-dropdown-content button')
		) {
			const variantArray = document.querySelectorAll(
				'div.clearfix.custom-dropdown-content button'
			);
			return variantArray;
		}
	});
	...
	await context.evaluate(async function () {
		const specificationsXpath =
			"//div[@id='mainProductDescription']//b[contains(text(),'Spécifications') or contains(text(),\"Caractéristiques\") or contains(text(),'Informations produit')]/following-sibling::text()|//h2[contains(text(),'Caractéristiques détaillées')]/following-sibling::node()|text()|//div[@id='mainProductDescription']//text()[contains(.,\"Dimensions\")]//following-sibling::text()[not(//div[@id='mainProductDescription']//b[contains(text(),'Spécifications') or contains(text(),\"Caractéristiques\") or contains(text(),'Informations produit')]/following-sibling::text()|//h2[contains(text(),'Caractéristiques détaillées')]/following-sibling::node()|text())] | //tr[contains(.,\"Dimensions\")]/following-sibling::tr[following::tr[contains(.,\"Contenu du carton\")]] | //tr[contains(.,\"Dimensions\")] | //div[contains(@class,\"specification\")]/div[@class=\"flix-std-container-fluid\"]";
		function _x(STR_XPATH, context) {
			// gets nodes using xpath
		...
		_x(specificationsXpath, document).forEach((q) => {
			specificationsText.push(q.textContent);
		});
		document.body.insertAdjacentHTML(
			'afterbegin',
			`<div id="specifications" style="display : none">${specificationsText.join(
				' '
			)}</div>`
		);
	});
	...
	await context
		.waitForXPath(
			'//div[@id="pdpFlixmediaZone" and not(contains(@style,"none"))]',
			{ timeout: 30000 }
		)
		.catch(() => {
		...
		const isVisible = (element) =>
			document.querySelector(element)
				? !!(
						...
						document.querySelector(element).offsetHeight
				  )
				: false;

		const content = {};
		content.description =
			document.querySelector('#inpage_container') &&
			isVisible('#inpage_container')
				? document.querySelector('#inpage_container').innerText
				: '';
