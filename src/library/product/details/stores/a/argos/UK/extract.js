  ...
  implementation: async (inputs, parameters, context, dependencies) => {
    ...
    async function waitForAplus () {
      ...
      await context.evaluate(async function () {
        const element = document.querySelector('section[data-test="product-alternatives"]');
        if (element) {
      ...
      try {
        await context.waitForXPath('//div[@data-test="alternatives-product-card"]');
      } catch (error) {
    ...
    const preparePage = async () => {
      ...
      try {
        const cookieBtnCss = '.privacy-prompt-footer a';
        console.log('Clicking on cookies button, cookieBtnCss: ' + cookieBtnCss);
        await context.waitForSelector(cookieBtnCss, { timeout: defaultTimeOutInMS });
        await context.click(cookieBtnCss);
      ...
      await context.evaluate(async function (zip) {
        ...
        const makeApiCall = async (url, options) => {
          try {
            ...
            if (!options) {
              ...
              console.log(`API statusCode: ${response.status}`);
        ...
        const getAvailability = async () => {
          ...
          console.log('availabilityResopnse from API', response);
          ...
          console.log(`availabilityResopnse text from API: ${availability}`);
          ...
          while (retryCount < retryLimit && !evaluationState) {
            try {
              ...
              if (retryCount === retryLimit) {
                ...
                console.log('availability API failed, will extract default value');
              }
            }
          }
        };
