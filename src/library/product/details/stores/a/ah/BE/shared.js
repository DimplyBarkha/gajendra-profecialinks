...
const transform = (data) => {
  ...
  data.forEach(el => {
    el.group.forEach(gr => {
      ...
      gr.sub_Category = gr.sub_Category.splice(2, 10);
      gr.price_per_unit[0].text = '€ ' + onlyNumbersAndDot(gr.price_per_unit[0].text);
      gr.price_per_unit_uom[0].text = gr.price_per_unit_uom[0].text.match(/(\b[A-Z][A-Z]+|\b[A-Z]\b)/g).join();
      gr.harvested_price = [{ text: '€ ' + gr.harvested_price.map(e => e.text).join('') }];
      gr.online_price = [{ text: gr.online_price.map(e => e.text).join('') }];
      gr.online_price_currency[0].text = gr.online_price_currency[0].text = '€';
      const indexSpace = gr.size[0].text.indexOf(' ');
      gr.size[0].text = gr.size[0].text.slice(0, indexSpace + 2);
      gr.net_weight[0].text = gr.net_weight[0].text.slice(0, indexSpace + 2);
      gr.storage = [gr.storage.find(e => e.text.includes('Bewaren')).text.slice(8, 200)];
