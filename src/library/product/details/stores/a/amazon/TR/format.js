...
const transform = (data, context) => {
  ...
  for (const { group } of data) {
    for (const row of group) {
      ...
      if (row.sold && row.sold[0] && row.ship && row.ship[0]) {
        row.shippingInfo = [{ text: row.sold[0].text === row.ship[0].text ? `${row.sold[0].text} tarafından satılır ve gönderilir.` : `gönderilir tarafindan ${row.ship[0].text} ve satılır tarafindan ${row.sold[0].text}` }];
      }
