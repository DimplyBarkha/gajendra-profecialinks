...
module.exports = {
  ...
  implementation: async ({ inputString }, { country, domain, transform: transformParam }, context, { productDetails }) => {
    ...
    const iframeUrl = await context.evaluate(async function () {
      const gtin = document.querySelector('span[itemprop*=gtin]') ? document.querySelector('span[itemprop*=gtin]').innerText.trim() : '';
      if (!gtin) {
    ...
    if (iframeUrl) {
      ...
      enhancedContent = await context.evaluate(async function () {
        const content = {};
        content.images = [];
        const imageNodes = document.querySelectorAll('img[class=pic]');
        ...
        content.videos = [];
        const videoNodes = document.querySelectorAll('*[data-video]');
      ...
      inBox = await context.evaluate(async function () {
        const content = {};
        content.images = [];
        const imageNodes = document.querySelectorAll('div.in-the-box img');
        imageNodes.forEach(img => {
          if (img.hasAttribute('data-src')) {
            content.images.push(img.getAttribute('data-src'));
          }
        });
        content.text = [];
        const boxTextNodes = document.querySelectorAll('div.in-the-box p');
        boxTextNodes.forEach(element => {
          content.text.push(element.innerText);
        });
        return content;
      });
    }
    ...
    await context.evaluate(async function (enhancedContent, inBox) {
      ...
      if (inBox.images) {
        inBox.images.forEach(q => {
          addElementToDocument('ii_inBoxImg', q);
        });
      }
      if (inBox.text) {
        inBox.text.forEach(q => {
          addElementToDocument('ii_inBoxText', q);
        });
      }
