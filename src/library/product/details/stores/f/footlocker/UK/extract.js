  ...
  implementation: async function implementation (
    ...
    try {
      ...
      await context.evaluate(() => {
        ...
        const stock = document.querySelector('div[class="fl-product-error-messages"]');
        if (stock.textContent.includes('In') || stock.textContent.includes('Less then')) {
          stock.setAttribute('availability', 'In stock');
        } else {
          stock.setAttribute('availability', 'Out of stock');
        }
      });
    } catch (err) {
