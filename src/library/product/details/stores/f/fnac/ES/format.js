...
const transform = data => {
  ...
  for (const {group} of data) {
    for (const row of group) {
      ...
      if (row.attributes) {
        row.attributes.forEach (elm => {
          elm.text = elm.text.replace (/[\n\s]+/, ' : ');
        });
      }
      if (row.videos) {
        const videos = Array.from (
          new Set (row.videos.map (elm => elm.text.trim ()))
        );
      ...
      if (row.discount) {
        ...
        row.promotion = [
          {
            text: `Antes ${listPrice.trim ()} ahora ${price.trim ()}, Oportunidad ${discount.trim ()}`,
          },
        ];
      }
      ...
      if (row.description) {
        ...
        row.productDescriptionWordCount = [
          {text: row.description[0].text.split (' ').length},
        ];
      }
    }
  }
