...
async function implementation(inputs, parameters, context, dependencies) {
  ...
  let videoDivInfoElmXpath = '//div[contains(@class,"VideoPlayerPlaylist-list")]//div[@data-video-count > 0]';
  try {
  ...
  try {
    allVideos = await context.evaluate(async (videoDivInfoElmXpath) => {
      let videoInfoElm = document.evaluate(videoDivInfoElmXpath, document, null, 7, null);
      let allVidArr = [];
      if(videoInfoElm && (videoInfoElm.snapshotLength > 0)) {
        let thisVideoInfoElement = videoInfoElm.snapshotItem(0);
        let totalVideoCount = thisVideoInfoElement.hasAttribute('data-video-count') ? thisVideoInfoElement.getAttribute('data-video-count') : '';
        totalVideoCount = parseInt(totalVideoCount);
        for(let i = 0; i < totalVideoCount; i++) {
          let thisUrl = thisVideoInfoElement.hasAttribute(`data-video-${i}-url`) ? thisVideoInfoElement.getAttribute(`data-video-${i}-url`) : '';
          console.log(thisUrl);
