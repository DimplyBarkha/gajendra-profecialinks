    ...
    dataObj.group.forEach((fieldName, index) => {
      ...
      if (fieldName.ratingCount) {
        ...
        if (infoObj.aggregateRating) {
          const str = infoObj.aggregateRating.reviewCount || '';
      ...
      if (fieldName.reviewCount) {
        const infoObj = JSON.parse(fieldName.reviewCount[0].text);
        if (infoObj.aggregateRating) {
          const str = infoObj.aggregateRating.reviewCount || '';
          fieldName.reviewCount[0].text = str;
        } else {
          fieldName.reviewCount[0].text = '';
        }
      }
