        ...
        for (let row of group) {
            ...
            if(row.specifications){
                ...
                row.specifications.forEach(item=>{
                    arr_info.push(item.text.replace(/(\s*\n\s*)+/g, ' : ').trim());            
                });                
                row.specifications = [{'text':arr_info.join(' || ')}];
            }
            if (row.manufacturer) {            
                row.manufacturer.forEach(item => {
                    ...
                    if (json_data['manufacturer']){
                        item.text = json_data['manufacturer'];
                    }
                    else{
            ...
            if (row.brandText) {
                row.brandText.forEach(item => {
                    try {
                    let json_data = JSON.parse(item.text);                    
                    if (json_data['brand']['name']){
                        item.text = json_data['brand']['name'];
                    }
                    else{
            ...
            if (row.variantId) {
                row.variantId.forEach(item => {
                    try {
                    let json_data = JSON.parse(item.text);                
                    if (json_data['productID']){
                        item.text = json_data['productID'];
                    }
                    else{
