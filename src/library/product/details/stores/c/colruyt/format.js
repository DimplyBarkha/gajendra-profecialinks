    ...
    for (const row of group) {
      ...
      if (row.servingSize) {
        row.servingSize.forEach((servingSizeItem) => {
          servingSizeItem.text = servingSizeItem.text.replace('Quantité moyenne de minéraux par 100g/100ml', '100ml').replace(/[^\d]/gm, '');
        });
      }
      if (row.servingSizeUom) {
        row.servingSizeUom.forEach((servingSizeUomItem) => {
          servingSizeUomItem.text = servingSizeUomItem.text.replace('Valeur nutritive moyenne par', '').replace('Quantité moyenne de minéraux par 100g/100ml', '100ml').replace(/[\d.]/gm, '');
        });
      }
