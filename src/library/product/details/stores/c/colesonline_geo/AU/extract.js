...
async function implementation (
  ...
  await context.evaluate(async () => {
    ...
    addHiddenDiv('ii_servingSize', servingSize.replace(/([\d]+(?:.[\d]+)?)(?:\s*)([a-zA-Z]*)\s{0,1}/, '$1 ').trim());
  ...
  await context.evaluate(async ({zipcode, storeId}) => {
    function addHiddenDiv (id, content) {
      const newDiv = document.createElement('div');
      newDiv.id = id;
      newDiv.textContent = content;
    ...
    zipcode && addHiddenDiv('pd_retailer_id', zipcode);
    storeId && addHiddenDiv('pd_store_id', storeId);
  }, inputs);
