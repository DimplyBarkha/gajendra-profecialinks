...
module.exports = {
  ...
  implementation: async ({ inputString }, { transform }, context, { productDetails }) => {
    await context.evaluate(async function () {
      ...
      // @ts-ignore
      data.warranty = [...document.querySelectorAll('dl[data-property-name*=Garantie] dd')].map(el => el ? el.innerText : '').join(' ');
      // @ts-ignore
      const variantTypes = [...document.querySelectorAll('div[class*=js-multi-discriminant-field]')].map(el => {
        return el.querySelector('strong') ? el.querySelector('strong').innerText : `${el.querySelector('strong').innerText}:`;
      });
      // @ts-ignore
      ...
      const dropdownVariants = [...document.querySelectorAll('div.collection-entrance__dropdown div.collection-entrance-card')].map(el => {
        return el.querySelector('strong') ? el.querySelector('strong').innerText : null;
      });
      let variants = [];
