const implementation = async (inputs, parameters, context, dependencies) => {
  ...
  // video
  await context.evaluate(function () {
    const iframe = document.querySelector('div.wc-video-gallery > iframe#wcframable1-0') ? document.querySelector('div.wc-video-gallery > iframe') : '';
    if (iframe !== '') {
      // @ts-ignore
  ...
  await context.evaluate(function () {
    const videoButton = document.querySelector('button.wc-ribbon-button');
    if (videoButton) {
      // @ts-ignore
      videoButton.click();
    }
  ...
  const videoLink = await context.evaluateInFrame('iframe[id="wcframable1-0"]', function () {
    return document.querySelector('video[aria-label="video player"]').getAttribute('src');
  });
