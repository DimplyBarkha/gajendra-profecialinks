...
module.exports = {
  ...
  implementation: async ({ parentInput }, { country, domain, transform: transformParam }, context, { productDetails }) => {
    ...
    if (videos && videos.length) {
      for (let i = 0; i < videos.length; i++) {
        ...
        if (selectorCheck) {
          ...
          const closeCheck = await context.evaluate(function () {
            const sel = document.querySelector('button[data-at="modal_close"]');
            if (sel) {
              sel.click();
              return true;
            } else {
    ...
    const variantArray = await context.evaluate(function (parentInput, html) {
      ...
      if (prodUrl[0]) {
        const urlUpdate = prodUrl[0].replace(/.com/g, '.com/ca/en');
