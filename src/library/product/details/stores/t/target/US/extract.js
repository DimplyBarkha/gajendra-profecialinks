async function implementation(
    ...
    try {
        ...
        const showMoreSelector = 'button[data-test="manufacturer-notes-button"]';
    ...
    await context.evaluate(() => {
        ...
        if (syndiPowerpage) {
            ...
            headings.forEach(h2 => {
                if (h2.innerText.includes('In the box') || h2.innerText.includes('In The Box') || h2.innerText.includes('in the box') || h2.innerText.includes('In the Box')) {
                    const parent = h2.parentElement;
                    const inTheBoxEls = parent.querySelectorAll('.syndigo-featureset-feature');
                    inTheBoxEls.forEach(el => {
                        const imgs = el.querySelector('img').getAttribute('src');
                        inTheBoxUrl = inTheBoxUrl + (inTheBoxUrl ? ' || ' : '') + imgs;
                        // @ts-ignore
