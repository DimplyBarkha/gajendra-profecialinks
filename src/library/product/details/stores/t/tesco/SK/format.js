    ...
    for (const row of group) {
      ...
      if (row.countryOfOrigin) {
        row.countryOfOrigin.forEach(item => {
          item.text = item.text.replace('Krajina pôvodu:', '');
          item.text = item.text.trim();
        });
      }
      if (row.packSize) {
        row.packSize.forEach(item => {
          item.text = item.text.replace('Počet jednotiek -', '');
          item.text = item.text.trim();
        });
      }
      if (row.quantity) {
        row.quantity.forEach(item => {
          item.text = item.text.replace('Číselná veľkosť -', '');
          item.text = item.text.trim();
        });
      }
      if (row.alcoholContent) {
        row.alcoholContent.forEach(item => {
          item.text = item.text.replace('Krajina pôvodu:', '');
          item.text = item.text.trim();
        });
      }
