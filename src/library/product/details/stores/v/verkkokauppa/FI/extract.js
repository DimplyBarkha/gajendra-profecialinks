...
module.exports = {
  ...
  implementation: async (inputs, { country, domain, transform: transformParam }, context, { productDetails }) => {
    try {
      ...
      const rpc = inputs.id;
      const prodPage = await context.evaluate(async function (rpc) {
        let prodPage = false;
        function stall (ms) {
        ...
        if (document.querySelectorAll('ol[class*="product-list-detailed"]>li')) {
          ...
          for (let i = 0; i < prodList.length; i++) {
            ...
            if (prodList[i].querySelector('img')) {
              ...
              if (prodCode.includes(rpc)) {
                prodList[i].querySelector('a[class*="list-product"]').click(); await stall(3000);
              }
            }
            if (prodList[i].querySelector('a[class*="thumbnail-link"]')) {
              prodCode = prodList[i].querySelector('a[class*="thumbnail-link"]').getAttribute('href');
              if (prodCode.includes(rpc)) { prodList[i].querySelector('a[class*="list-product"]').click(); await stall(3000); }
            }
          }
        }

        if (document.querySelector('ul[class*="breadcrumbs"]')) { prodPage = true; }
        return prodPage;
      }, rpc);
