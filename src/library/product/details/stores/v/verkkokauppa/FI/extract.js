...
module.exports = {
  ...
  implementation: async (inputs, { country, domain, transform: transformParam }, context, { productDetails }) => {
    try {
      const productUrl = await context.evaluate(async function (inputs) {
        try {
          const a = await fetch('https://web-api.service.verkkokauppa.com/search?query=' + inputs.id + '&sort=score%3Adesc&private=true&sessionId=cac15ad6-4832-4c2e-8f26-035e7ba0e123');
          const data = await a.json();
          const products = data.products.length;
          let productUrl = '';
          for (let i = 0; i < products; i++) {
            console.log('current product id ', data.products[i].pid);
            if (data.products[i].pid == inputs.id) {
              productUrl = data.products[i].href.fi;
            }
          }
          productUrl = 'https://www.verkkokauppa.com' + productUrl;
          return productUrl;
        } catch (err) {
      ...
      try {
        await context.waitForSelector('#allow-cookies');
