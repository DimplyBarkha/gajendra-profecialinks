  ...
  await context.evaluate(async () => {
    if (document.querySelector('div[role="alertdialog"]') === null) {
      const price = document.querySelector(
        'div[id="productDetailsPanel"] div[class="product-price-panel__price-alt-vat"]')
      ...
      document
        .querySelector('body')
        .setAttribute('price', price.match(regex)[1]);
      ...
      document.querySelector('div[role="alertdialog"]').textContent ===
      'Helaas is dit product niet langer beschikbaar'
    ) {
      document
        .querySelector('body')
        .setAttribute('availabilityText', 'Out of Stock');
    }
  });
