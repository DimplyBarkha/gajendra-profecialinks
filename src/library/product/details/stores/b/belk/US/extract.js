...
async function implementation(
  ...
  await context.evaluate(async function () {
    const rpcElememt = document.evaluate('//div[@class="inventory"]//div[@class="product-variations"]', document).iterateNext()
    const jsonData = rpcElememt.getAttribute('data-attributes')
    const data = JSON.parse(jsonData)
    const rpc = data.color.value
    const appendDiv = document.querySelector('h1[class="product-name"] div[itemprop="name"]')
    appendDiv.setAttribute('rpcValue', rpc)
