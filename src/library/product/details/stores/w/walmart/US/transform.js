  ...
  for (const { group } of data) {
    for (const row of group) {
      try {
        ...
        if (row.description) {
          const text = row.description.map(elm => elm.text.trim()).filter(elm => elm).join(' ');
        ...
        if (!row.productDimension && row.specObject) {
          // remove json if present
          const dimension = JSON.parse(row.specObject[0].text).find(elm => elm['Assembled Product Dimensions (L x W x H)']);
          if (dimension) {
            row.productDimension = [{ text: dimension['Assembled Product Dimensions (L x W x H)'] }];
          }
        }
        if (row.specObject) {
        ...
        if (row.profiteroDescription) {
          row.productDescriptionLength = [{ text: row.profiteroDescription[0].text.length }];
          row.productDescriptionWordCount = [{ text: row.profiteroDescription[0].text.split(' ').length }];
        }
