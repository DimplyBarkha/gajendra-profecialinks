    ...
    for (const row of group) {
      ...
      if (row.manufacturerImages) {
        ...
        row.manufacturerImages.forEach(item => {
          ...
          const preview = item.text.includes('preview');
          const loading = item.text.includes('.gif');
          if (val < 0 && !preview && !loading) {
        ...
        if (temp > 0) {
          ...
          // @ts-ignore
          let manImages = [...new Set(images)];
          let text2 = manImages.join(' | ');
