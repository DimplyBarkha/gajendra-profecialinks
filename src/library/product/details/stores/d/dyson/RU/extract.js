...
async function implementation(
    ...
    await context.evaluate(async function() {
        ...
        let getSkus = geturls.split('/')
        let getSku = getSkus[getSkus.length - 2]
        let skus = getSku.split('-')
        let sku = skus[skus.length - 2] + '-' + skus[skus.length - 1]
        document.head.setAttribute('sku', sku);
        let getRating = document.querySelector('div[itemprop="aggregateRating"] meta[itemprop="ratingValue"]') ? document.querySelector('div[itemprop="aggregateRating"] meta[itemprop="ratingValue"]').getAttribute('content') : '';
        if (getRating)
            getRating = getRating.replace('.', ',');
        document.head.setAttribute('rating', getRating)
    });
