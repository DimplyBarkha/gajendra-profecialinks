  ...
  await context.evaluate(async function () {
    ...
    if (skulist) {
      for (let i = 0; i < skulist.length; i++) {
        ...
        //         //if(document.querySelector('button[@id="add-to-cart")')) array[i].setAttribute("Availability",'present');
        //         array[i].setAtt
        //         break
        //       }
        //     }
        ...
        if (document.querySelector('button[id*="add-to-cart"]')) array[i].setAttribute('Availability', 'present');
        try {
          const promotion = getElementByXpath('//div[contains(@class,"product-price-summary")]//div[@itemprop="offers"]/div[2]/..');
          if (promotion) {
            const promoText = promotion.textContent;
            array[i].setAttribute('promos', promoText);
          }
        } catch (e) { console.log('Promotions not present'); }
        if (skulist[i].id) {
