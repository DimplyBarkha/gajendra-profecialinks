...
const transform = (data, context) => {
  ...
  for (const { group } of data) {
    for (const row of group) {
      ...
      if (!row.aggregateRating) {
        if(row.aggregateRating.length > 1){
          row.aggregateRating = row.aggregateRating[0]
        }
      }
      if (!row.sku) {
        if(row.sku.length > 1){
  ...
  data = data.filter(function (item) {
    ...
    item.group = item.group.filter(function (row) {
      ...
      if ( row.reviewDate && row.reviewDate.length && checkIfReviewIsFromLast30Days(row.reviewDate[0].text, maxDays)) {
        return true;
      }
      return false;
    });
