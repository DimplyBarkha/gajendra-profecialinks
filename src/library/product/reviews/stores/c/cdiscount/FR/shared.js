...
const transform = (data, context) => {
    ...
    function checkIfReviewIsFromLast30Days (reviewDate, maxDays) {
      ...
      // date format used below is "MM/DD/YYYY" 
      let formattedData = reviewDate.split('/');
      formattedData = `${formattedData[1]}/${formattedData[0]}/${formattedData[2]}`
      if (new Date(formattedData).getTime() >= timestamp) {
        return true;
      }
      return false;
    }
