...
/**
 *
 * @param { { URL: string, id: any, RPC: string, SKU: string, date: string, days: number } } inputs
...
async function implementation (
  ...
  let { URL, RPC, SKU, date = null, days = 30 } = inputs;
  ...
  console.log(JSON.stringify(resultsReturned));
  if (!resultsReturned) {
    console.log('No results were returned');
    return;
  }
  if (!date && days) {
    date = new Date().setDate(new Date().getDate() - days).toString();
  }
  const pageOne = await extract({ date });
  ...
  let page = 2;
  while (await paginate({ id, page, offset: collected, date })) {
    const { data, stop } = await extract({ date });
